...
CREATE TABLE IF NOT EXISTS bronze.raw_job_postings (
    -- Auto-generated unique identifier for each raw job record.
    raw_job_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,

    -- Links this raw record to the pipeline run that created it.
    pipeline_run_id BIGINT NOT NULL,

    -- Name of the source.
    -- Example: "linkedin", "indeed", "sample_other_site"
    source_name TEXT NOT NULL,

    -- Original job URL if available.
    -- This is nullable because some sources may not provide a stable URL.
    source_url TEXT,

    -- When the scraper collected this record.
    -- This may come from the scraper.
    scraped_at TIMESTAMPTZ,

    -- When the record was inserted into the database.
    -- This is set automatically by PostgreSQL.
    ingested_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,

    -- The complete raw scraped object.
    -- This is the source of truth for the bronze layer.
    raw_payload JSONB NOT NULL,

    -- Hash of raw_payload used for duplicate detection.
    -- This should be generated by the Python loader.
    content_hash TEXT NOT NULL,

    -- Foreign key linking raw records to pipeline runs.
    -- If a pipeline run is deleted, its raw records are deleted too.
    CONSTRAINT fk_raw_job_postings_pipeline_run
        FOREIGN KEY (pipeline_run_id)
        REFERENCES metadata.pipeline_runs(pipeline_run_id)
        ON DELETE CASCADE,

    -- Prevent inserting the same raw content twice for the same source.
    -- This is more reliable than relying only on source_url.
    CONSTRAINT uq_raw_job_content_hash
